...
def detect_tx_type(tx, logs):
    ...
    swap_methods = ["0x38ed1739", "0x7ff36ab5", "0x18cbafe5", "0x791ac947", "0x5c11d795", "0x414bf389", "0xdb3e2198", "0xc04b8d59"]
    stake_methods = ["0xa694fc3a", "0x3a4b66f1", "0x1249c58b", "0xb6b55f25", "0xe2bbb158"]
    unstake_methods = ["0x2e1a7d4d", "0x38d07436", "0xd1058e59", "0x441a3e70", "0x853828b6"]
    ...
    try:
        ...
        for tx in data.get("txs", []):
            ...
            if tx_type:
                emoji = type_emoji
                direction = tx_type
                label = WALLET_LABELS.get(from_addr) or WALLET_LABELS.get(to_addr) or from_addr
            elif to_addr in WALLET_LABELS:
                emoji = "🟢"
                direction = "RECEIVED"
                label = WALLET_LABELS[to_addr]
            elif from_addr in WALLET_LABELS:
                emoji = "🔴"
                direction = "SENT"
                label = WALLET_LABELS[from_addr]
            else:
                emoji = "🔄"
                direction = "ACTIVITY"
                label = to_addr if to_addr else from_addr
